...
liverGroups <- liverPhenotypes$animal.key.anirandgroup
liverGroupsFactor <- fct_infreq(factor(liverGroups))

####Split data into experimental groups

ControlIPEIDs <- as.character(intersect(colnames(liverCounts), ControlIPEPhenotypes$vial_label))
ControlIPEData <- dplyr::select(liverCounts, ControlIPEIDs)

Control7hrIDs <- as.character(intersect(colnames(liverCounts), Control7hrPhenotypes$vial_label))
Control7hrData <- dplyr::select(liverCounts, Control7hrIDs)

ExerciseIPEIDs <- as.character(intersect(colnames(liverCounts), ExerciseIPEPhenotypes$vial_label))
ExerciseIPEData <- dplyr::select(liverCounts, ExerciseIPEIDs)

Exercise0.5hrIDs <- as.character(intersect(colnames(liverCounts), Exercise0.5hrPhenotypes$vial_label))
Exercise0.5hrData <- dplyr::select(liverCounts, Exercise0.5hrIDs)

Exercise1hrIDs <- as.character(intersect(colnames(liverCounts), Exercise1hrPhenotypes$vial_label))
Exercise1hrData <- dplyr::select(liverCounts, Exercise1hrIDs)

Exercise4hrIDs <- as.character(intersect(colnames(liverCounts), Exercise4hrPhenotypes$vial_label))
Exercise4hrData <- dplyr::select(liverCounts, Exercise4hrIDs)

Exercise7hrIDs <- as.character(intersect(colnames(liverCounts), Exercise7hrPhenotypes$vial_label))
Exercise7hrData <- dplyr::select(liverCounts, Exercise7hrIDs)

Exercise24hrIDs <- as.character(intersect(colnames(liverCounts), Exercise24hrPhenotypes$vial_label))
Exercise24hrData <- dplyr::select(liverCounts, Exercise24hrIDs)

Exercise48hrIDs <- as.character(intersect(colnames(liverCounts), Exercise48hrPhenotypes$vial_label))
Exercise48hrData <- dplyr::select(liverCounts, Exercise48hrIDs)
...
save(res, file = "liver_IPECtrl.RData")
...
setwd("/Users/Chigoziri/MoTrPAC/heart0.5hr")
save(res, file = "liver_0.5hrCtrl.RData")
...
setwd("/Users/Chigoziri/MoTrPAC/heart1hr")
save(res, file = "liver_1hrCtrl.RData")
...
setwd("/Users/Chigoziri/MoTrPAC/heart4hr")
save(res, file = "liver_4hrCtrl.RData")
...
setwd("/Users/Chigoziri/MoTrPAC/heart7hr")
save(res, file = "liver_7hrCtrl.RData")
...
setwd("/Users/Chigoziri/MoTrPAC/heart24hr")
save(res, file = "liver_24hrCtrl.RData")
...
setwd("/Users/Chigoziri/MoTrPAC/heart48hr")
save(res, file = "liver_48hrCtrl.RData")

####Perform KEGG enrichment for all time points and put data together for dot plots####

setwd("/Users/Chigoziri/MoTrPAC/liverCtrl")
load("liver_IPECtrl.RData")
...
setwd("/Users/Chigoziri/MoTrPAC/liver0.5hr")
load("liver_0.5hrCtrl.RData")
...
setwd("/Users/Chigoziri/MoTrPAC/liver1hr")
load("liver_1hrCtrl.RData")
...
setwd("/Users/Chigoziri/MoTrPAC/liver4hr")
load("liver_4hrCtrl.RData")
...
setwd("/Users/Chigoziri/MoTrPAC/liver7hr")
load("liver_7hrCtrl.RData")
...
setwd("/Users/Chigoziri/MoTrPAC/liver24hr")
load("liver_24hrCtrl.RData")
...
setwd("/Users/Chigoziri/MoTrPAC/liver48hr")
load("liver_48hrCtrl.RData")
...
saveWorkbook(ekeggTimeCourseWB, "liverTimeCourse_enrichKEGG_FCcutoff1_0.05.xlsx")
saveWorkbook(ekeggEnrichedPathwaysWB,"liverTimeCourse_enrichedPathways_FCcutoff1.xlsx")
...
ekeggDotPlotData_filt <- ekeggDotPlotData[ ( !grepl("Epstein-Barr virus infection", ekeggDotPlotData$Description)) & 
                                             ...
                                             ( !grepl("Asthma", ekeggDotPlotData$Description)) & 
                                             ( !grepl("Herpes simplex virus 1 infection", ekeggDotPlotData$Description)) & 
                                             ( !grepl("Human T-cell leukemia virus 1 infection", ekeggDotPlotData$Description)) & 
                                             ( !grepl("Insulin resistance", ekeggDotPlotData$Description)) & 
                                             ( !grepl("Non-alcoholic fatty liver disease", ekeggDotPlotData$Description)) & 
                                             ( !grepl("Staphylococcus aureus infection", ekeggDotPlotData$Description)) & 
                                             ( !grepl("Systemic lupus erythematosus", ekeggDotPlotData$Description)) & 
                                             ( !grepl("Toxoplasmosis", ekeggDotPlotData$Description)) & 
                                             ( !grepl("Type I diabetes mellitus", ekeggDotPlotData$Description)) & 
                                             ( !grepl("Viral myocarditis", ekeggDotPlotData$Description)) & 
                                             ( !grepl("Prion disease", ekeggDotPlotData$Description)) & 
                                             ( !grepl("Gastric cancer", ekeggDotPlotData$Description)), ] 
...
tiff("liverTimeDirection_DotPlot_FCcutoff1_supp_0.05.tiff", units="in", width=11, height=8, res=300)

liverDotPlot <- ggplot(ekeggDotPlotData_supp, aes(x= Time, y=reorder(Description, dplyr::desc(Description)), size=GeneRatio, color=medianFC, group=Time)) + geom_point(alpha = 0.8) + 
  theme_bw(base_size = 18) + guides(color=guide_colourbar(order=1),size=guide_legend(order=2)) + ylab("Description")
liverDotPlot = liverDotPlot+scale_color_gradient(low = "red2",  high = "mediumblue", space = "Lab", limit = c(min(ekeggDotPlotData$medianFC),max(ekeggDotPlotData$medianFC)))
liverDotPlot+scale_size(range = c(2, 8))

dev.off()

####

tiff("liverTimeDirection_filt_DotPlot_FCcutoff1_0.05.tiff", units="in", width=12, height=8, res=300)

liverDotPlot <- ggplot(ekeggDotPlotData_filt, aes(x= Time, y=reorder(Description, dplyr::desc(Description)), size=GeneRatio, color=medianFC, group=Time)) + geom_point(alpha = 0.8) + 
  theme_bw(base_size = 18) + guides(color=guide_colourbar(order=1),size=guide_legend(order=2)) + ylab("Description")
liverDotPlot = liverDotPlot+scale_color_gradient(low = "red2",  high = "mediumblue", space = "Lab", limit = c(min(ekeggDotPlotData_filt$medianFC),max(ekeggDotPlotData_filt$medianFC)))      
liverDotPlot+scale_size(range = c(2, 8))
