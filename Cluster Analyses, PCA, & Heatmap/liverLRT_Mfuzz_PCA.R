...
save(res_LRT, rld, rld_mat, coldata, file = "liver_LRTData.RData")
...
liver_eset <- ExpressionSet(assayData = dataAllTimesMat)

mest <- mestimate(liver_eset)

dataFuzz <- mfuzz(liver_eset,10,mest)

#Genes within the core of the cluster, with a membership score greater than 0.4
dataFuzz_acore <- acore(liver_eset,dataFuzz,min.acore=0.4) 

#save(dataFuzz, dataFuzz_acore, file="liver_dataFuzz.RData")
...
#write.csv(graphLabels,file="liver_MfuzzGraphLabels.csv")

#Plot clusters generated from Mfuzz
mfuzz.plot2(liver_eset,dataFuzz,mfrow=c(4,4),min.mem=0.4,time.labels=c("Ctrl","0","0.5","1","4","7","24","48"),
            xlab="Time Post-Exercise (hr)",cex.lab=1.5,cex.axis=1.5) +
  annotate("text", x=1, y=1, label= )
quartz.save(file="liverMfuzzClusters_1000_c8_min.men0.4_clusterNum_t.tiff",type="tiff",width=18, height=15, dpi=300)
dev.off()

#load("liver_dataFuzz.RData")
...
if (file.exists("liverMfuzzClusters_c10_1000.xlsx")==FALSE) {
  ...
  saveWorkbook(geneClustersWB, "liverMfuzzClusters_c10_1000.xlsx")
  ...
  saveWorkbook(ekeggClustersWB, "liverMfuzzClusters_c10_1000_enrichKEGG_0.05_FINAL.xlsx")
  saveWorkbook(ekeggEnrichedPathwaysWB,"liverMfuzzClusters_enrichedPathways.xlsx")
}
...
liverDotPlot = liverDotPlot+scale_color_gradient(low = "red2",  high = "mediumblue", space = "Lab", limit = c(0.00000000001,0.05))
...
tiff("liverLRTPCA.tiff", units="in", width=12, height=8, res=300)
