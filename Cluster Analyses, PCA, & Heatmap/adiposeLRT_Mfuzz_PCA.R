...
save(res_LRT, rld, rld_mat, coldata, file = "adipose_LRTData.RData")
...
#save(dataFuzz, dataFuzz_acore, file="adipose_dataFuzz.RData")
...
#write.csv(graphLabels,file="adipose_MfuzzGraphLabels.csv")

#Plot clusters generated from Mfuzz
mfuzz.plot2(adipose_eset,dataFuzz,mfrow=c(4,4),min.mem=0.4,time.labels=c("Ctrl","0","0.5","1","4","7","24","48"),
            xlab="Time Post-Exercise (hr)",cex.lab=1.5,cex.axis=1.5) +
  annotate("text", x=1, y=1, label= )
quartz.save(file="adiposeMfuzzClusters_1000_c8_min.men0.4_clusterNum_t.tiff",type="tiff",width=18, height=15, dpi=300)
dev.off()

#load("adipose_dataFuzz.RData")
...
if (file.exists("adiposeMfuzzClusters_c10_1000.xlsx")==FALSE) {
  ...
  saveWorkbook(geneClustersWB, "adiposeMfuzzClusters_c10_1000.xlsx")
  ...
  saveWorkbook(ekeggClustersWB, "adiposeMfuzzClusters_c10_1000_enrichKEGG_0.05_FINAL.xlsx")
  saveWorkbook(ekeggEnrichedPathwaysWB,"adiposeMfuzzClusters_enrichedPathways.xlsx")
}

names(ekeggDotPlotData) <- c("Description","GeneRatio","padj","Cluster")

#Convert GeneRatio from fraction character to decimal
ekeggDotPlotData$Cluster <- factor(ekeggDotPlotData$Cluster,levels=c("Cluster 1","Cluster 2","Cluster 3","Cluster 4","Cluster 5","Cluster 6"))
#Convert GeneRatio from fraction character to decimal
ekeggDotPlotData$GeneRatio <- sapply(ekeggDotPlotData$GeneRatio, function(x) eval(parse(text=x))) 

#Save dot plot as .tiff
tiff("adiposeMfuzzClusters_c8_1000_DotPlot_0.05.tiff", units="in", width=12, height=10, res=300)

adiposeDotPlot <- ggplot(ekeggDotPlotData, aes(x= Cluster, y=reorder(Description, dplyr::desc(Description)), size=GeneRatio, color=padj, group=Cluster)) + geom_point(alpha = 0.8) + 
  theme_bw(base_size = 14) + ylab("Description")
adiposeDotPlot = adiposeDotPlot+scale_color_gradient(low = "red2",  high = "mediumblue", space = "Lab", limit = c(0.00000000001,0.05))
...
tiff("adiposeLRTPCA.tiff", units="in", width=12, height=8, res=300)
