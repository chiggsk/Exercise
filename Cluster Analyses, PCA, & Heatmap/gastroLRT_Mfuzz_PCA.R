...
gastro_eset <- ExpressionSet(assayData = dataAllTimesMat)

mest <- mestimate(gastro_eset)

dataFuzz <- mfuzz(gastro_eset,10,mest)

#Genes within the core of the cluster, with a membership score greater than 0.4
dataFuzz_acore <- acore(gastro_eset,dataFuzz,min.acore=0.4) 

#save(dataFuzz, dataFuzz_acore, file="gastro_dataFuzz.RData")
...
#write.csv(graphLabels,file="gastro_MfuzzGraphLabels.csv")

#Plot clusters generated from Mfuzz
mfuzz.plot2(gastro_eset,dataFuzz,mfrow=c(4,4),min.mem=0.4,time.labels=c("Ctrl","0","0.5","1","4","7","24","48"),
            xlab="Time Post-Exercise (hr)",cex.lab=1.5,cex.axis=1.5) +
  annotate("text", x=1, y=1, label= )
quartz.save(file="gastroMfuzzClusters_1000_c8_min.men0.4_clusterNum_t.tiff",type="tiff",width=18, height=15, dpi=300)
dev.off()

#load("gastro_dataFuzz.RData")
...
if (file.exists("gastroMfuzzClusters_c10_1000.xlsx")==FALSE) {
  ...
  saveWorkbook(geneClustersWB, "gastroMfuzzClusters_c10_1000.xlsx")
  ...
  saveWorkbook(ekeggClustersWB, "gastroMfuzzClusters_c10_1000_enrichKEGG_0.05_FINAL.xlsx")
  saveWorkbook(ekeggEnrichedPathwaysWB,"gastroMfuzzClusters_enrichedPathways.xlsx")
}
...
tiff("gastroMfuzzClusters_c8_1000_DotPlot_0.05.tiff", units="in", width=12, height=10, res=300)

gastroDotPlot <- ggplot(ekeggDotPlotData, aes(x= Cluster, y=reorder(Description, dplyr::desc(Description)), size=GeneRatio, color=padj, group=Cluster)) + geom_point(alpha = 0.8) + 
  theme_bw(base_size = 14) + ylab("Description")
gastroDotPlot = gastroDotPlot+scale_color_gradient(low = "red2",  high = "mediumblue", space = "Lab", limit = c(0.00000000001,0.05))
