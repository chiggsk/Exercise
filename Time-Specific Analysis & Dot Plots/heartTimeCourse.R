...
save(res, file = "heart_IPECtrl.RData")
...
save(res, file = "heart_0.5hrCtrl.RData")
...
save(res, file = "heart_1hrCtrl.RData")
...
save(res, file = "heart_4hrCtrl.RData")
...
save(res, file = "heart_7hrCtrl.RData")
...
save(res, file = "heart_24hrCtrl.RData")
...
saveWorkbook(ekeggTimeCourseWB, "heartTimeCourse_enrichKEGG_FCcutoff1_0.05.xlsx")
saveWorkbook(ekeggEnrichedPathwaysWB,"heartTimeCourse_enrichedPathways_FCcutoff1.xlsx")
...
ekeggDotPlotData_filt <- ekeggDotPlotData[ ( !grepl("Epstein-Barr virus infection", ekeggDotPlotData$Description)) & 
                                             ...
                                             ( !grepl("Non-alcoholic fatty heart disease", ekeggDotPlotData$Description)) & 
                                             ...
                                             ( !grepl("Endometrial cancer", ekeggDotPlotData$Description)) & 
                                             ( !grepl("Amoebiasis", ekeggDotPlotData$Description)) & 
                                             ( !grepl("Small cell lung cancer", ekeggDotPlotData$Description)), ] 
...
tiff("heartTimeDirection_DotPlot_FCcutoff1_supp_0.05.tiff", units="in", width=11, height=8, res=300)

heartDotPlot <- ggplot(ekeggDotPlotData_supp, aes(x= Time, y=reorder(Description, dplyr::desc(Description)), size=GeneRatio, color=medianFC, group=Time)) + geom_point(alpha = 0.8) + 
  theme_bw(base_size = 18) + guides(color=guide_colourbar(order=1),size=guide_legend(order=2)) + ylab("Description")
heartDotPlot = heartDotPlot+scale_color_gradient(low = "red2",  high = "mediumblue", space = "Lab", limit = c(min(ekeggDotPlotData$medianFC),max(ekeggDotPlotData$medianFC)))
...
tiff("heartTimeDirection_filt_DotPlot_FCcutoff1_0.05.tiff", units="in", width=12, height=8, res=300)

heartDotPlot <- ggplot(ekeggDotPlotData_filt, aes(x= Time, y=reorder(Description, dplyr::desc(Description)), size=GeneRatio, color=medianFC, group=Time)) + geom_point(alpha = 0.8) + 
  theme_bw(base_size = 18) + guides(color=guide_colourbar(order=1),size=guide_legend(order=2)) + ylab("Description")
heartDotPlot = heartDotPlot+scale_color_gradient(low = "red2",  high = "mediumblue", space = "Lab", limit = c(min(ekeggDotPlotData_filt$medianFC),max(ekeggDotPlotData_filt$medianFC)))      
heartDotPlot+scale_size(range = c(2, 8))
