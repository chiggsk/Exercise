...
save(res, file = "blood_IPECtrl.RData")
...
save(res, file = "blood_0.5hrCtrl.RData")
...
save(res, file = "blood_1hrCtrl.RData")
...
save(res, file = "blood_4hrCtrl.RData")
...
save(res, file = "blood_7hrCtrl.RData")
...
save(res, file = "blood_24hrCtrl.RData")
...
save(res, file = "blood_48hrCtrl.RData")
...
saveWorkbook(ekeggTimeCourseWB, "bloodTimeCourse_enrichKEGG_FCcutoff1_0.05.xlsx")
saveWorkbook(ekeggEnrichedPathwaysWB,"bloodTimeCourse_enrichedPathways_FCcutoff1.xlsx")
...
ekeggDotPlotData_filt <- ekeggDotPlotData[ ( !grepl("Epstein-Barr virus infection", ekeggDotPlotData$Description)) & 
                                             ...
                                             ( !grepl("Autoimmune thyroid disease", ekeggDotPlotData$Description)) & 
                                             ( !grepl("Graft-versus-host disease", ekeggDotPlotData$Description)), ] 
...
tiff("bloodTimeDirection_DotPlot_FCcutoff1_supp_0.05.tiff", units="in", width=11, height=8, res=300)

bloodDotPlot <- ggplot(ekeggDotPlotData_supp, aes(x= Time, y=reorder(Description, dplyr::desc(Description)), size=GeneRatio, color=medianFC, group=Time)) + geom_point(alpha = 0.8) + 
  theme_bw(base_size = 18) + guides(color=guide_colourbar(order=1),size=guide_legend(order=2)) + ylab("Description")
bloodDotPlot = bloodDotPlot+scale_color_gradient(low = "red2",  high = "mediumblue", space = "Lab", limit = c(min(ekeggDotPlotData$medianFC),max(ekeggDotPlotData$medianFC)))
...
tiff("bloodTimeDirection_filt_DotPlot_FCcutoff1_0.05.tiff", units="in", width=12, height=8, res=300)

bloodDotPlot <- ggplot(ekeggDotPlotData_filt, aes(x= Time, y=reorder(Description, dplyr::desc(Description)), size=GeneRatio, color=medianFC, group=Time)) + geom_point(alpha = 0.8) + 
  theme_bw(base_size = 18) + guides(color=guide_colourbar(order=1),size=guide_legend(order=2)) + ylab("Description")
bloodDotPlot = bloodDotPlot+scale_color_gradient(low = "red2",  high = "mediumblue", space = "Lab", limit = c(min(ekeggDotPlotData_filt$medianFC),max(ekeggDotPlotData_filt$medianFC)))      
bloodDotPlot+scale_size(range = c(2, 8))
